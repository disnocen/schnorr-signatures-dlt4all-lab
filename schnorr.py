...
# when you see
# "# insert code here" 
# or "..." you should complete the code 
...
def schnorr_sign(msg: bytes, seckey: bytes, aux_rand: bytes) -> bytes:
    ...
    k0 = int_from_bytes(tagged_hash("BIP340/nonce", ...)) % n
    if k0 == 0:
        raise RuntimeError('Failure. This happens only with negligible probability.')
    R = point_mul(G, k0)
    assert R is not None
    k = n - k0 if not has_square_y(R) else k0
    e = int_from_bytes(tagged_hash("BIP340/challenge", ...)) % n
    sig = # insert code here, adding bytes of R and e
...
def schnorr_verify(msg: bytes, pubkey: bytes, sig: bytes) -> bool:
    ...
    r = # insert code here, first 32 bytes of sig
    s = # insert code here, last 32 bytes of sig
